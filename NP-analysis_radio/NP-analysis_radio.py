...
def precision_recall_from_scores(y_true: np.ndarray, scores: np.ndarray):
    """
    Compute a precision–recall curve and average precision (AP) for a ranking by `scores`.

    Parameters
    ----------
    y_true : np.ndarray, shape (N,)
        Binary ground truth labels {0,1}. Here, 1 denotes "fragile" (a flip detected within budget).
    scores : np.ndarray, shape (N,)
        Ranking scores; higher means "more fragile" (e.g., |c4|).

    Returns
    -------
    rec : np.ndarray
        Recall values, starting at 0.0 (prepended).
    prec : np.ndarray
        Precision values, starting at 1.0 by convention (prepended).
    ap_step : float
        Step-integrated area under the PR curve (average precision).
    df_curve : pd.DataFrame
        Per-prefix table with columns:
        k, threshold(score), tp, fp, fn, tn, precision, recall.
    """
    y = np.asarray(y_true).astype(int)
    s = np.asarray(scores, dtype=float)

    # Drop invalid scores (keep NaN/inf out of the ranking).
    valid = np.isfinite(s)
    y = y[valid]
    s = s[valid]

    # Sort by score descending; mergesort for stability (ties keep original order).
    ...
    # Prepend (recall=0, precision=1) for plotting and AP via step integration.
...
# ---------- Robust float regex ----------
# Matches decimal / scientific notation, with optional sign.
FLOAT_RE = r'[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?'

# ---------- TXT parser ----------
def parse_benchmark_file(txt_path: str) -> dict:
    """
    Parse a single post_processing_*/benchmark_point*.txt file.

    Extracts:
      - kink diagnostics (frac_kink/active/inactive, samples_checked),
      - local fit diagnostics (kept_ratio, cond, rank, n_monomials, degree_used, retry),
      - approximation quality (RMSE/MAE/Pearson/Sign_Agreement + residual stats),
      - flip radii table (collect YES rows) and min_flip_radius,
      - timings (Puiseux total time, saliency timing/memory/grad norm),
      - optional predicted onset radius (r_dom_pred) if present in free-form notes,
      - optional axis-baseline sweep summaries (flip_grad/lime/shap).

    Parameters
    ----------
    txt_path : str
        Path to a benchmark TXT file.

    Returns
    -------
    dict
        Parsed fields; missing values default to NaN or empty list.
    """
    p = Path(txt_path)
    ...
    out = {
        ...
        "saliency_grad_norm": np.nan,  # trailing comma in prior logs was intentional; keep the key stable
    ...
    for raw in lines:
        ...
        # Optional r_dom line (free-form variants)
    ...
    # Robustness table → collect flip radii from "YES" rows
    in_table = False
    for raw in lines:
        s = raw.strip()
        s_norm = s.replace("—", "-")
...
def collect_benchmarks(pp_dir: Path) -> pd.DataFrame:
    """
    Load and parse all benchmark_point*.txt files from a post_processing_* directory.

    Parameters
    ----------
    pp_dir : Path
        Directory containing the post-processing outputs (TXT files).

    Returns
    -------
    pd.DataFrame
        One row per point with parsed metrics. Empty DataFrame if none found.
    """
...
def load_anchors(up_dir: Path) -> pd.DataFrame:
    """
    Load the anchor set (uncertain points) from up_*/uncertain_full.csv and
    add canonical columns used for joins and summaries.

    Parameters
    ----------
    up_dir : Path
        Directory containing 'uncertain_full.csv'.

    Returns
    -------
    pd.DataFrame
        Anchor table with 'point' (0..N-1), and optional pmax/margin if p1/p2 present.
    """
...
def load_dom_ratio(pp_dir: Path) -> pd.DataFrame:
    """
    Load dominant-ratio summary (if present) and normalize column names.

    Parameters
    ----------
    pp_dir : Path
        Directory containing 'dominant_ratio_summary.csv'.

    Returns
    -------
    pd.DataFrame
        Columns subset of ['point','c2_max_abs','c4_max_abs','r_dom','r_flip'] if available.
        Empty DataFrame if the CSV is missing.
    """
    ...
    df = df.rename(columns={
        ...
        "point_id": "point"  # if someone saved under 'point_id'
    })
...
def main():
    """
    End-to-end pipeline:
      1) Auto-detect input dirs (radio/real), and create output folders.
      2) Load anchors, TXT benchmarks, and dominant-ratio CSV (if present).
      3) Join on 'point'; reconcile r_flip_obs and r_dom fields.
      4) Save head-to-head flip summary and the joined CSV.
      5) Compute correlations, PR/AUPRC triage by different scores; save figures/tables.
      6) Produce a short markdown report with key stats and figure paths.
    """
    BASE = Path(__file__).resolve().parent  # build_np_evidence/

    # Auto-detect REAL vs RADIO
    # Priority: if *_radio exists use it; else try *_real
    UP_DIR = (BASE.parent / "up_radio") if (BASE.parent / "up_radio").exists() else (BASE.parent / "up_real")
    PP_DIR = (BASE.parent / "post_processing_radio") if (BASE.parent / "post_processing_radio").exists() else (BASE.parent / "post_processing_real")
    OUT_DIR = BASE
    ...
    # Sanity: check TXT vs anchors indexing
    ...
    if {"c4_max_abs", "r_flip_obs"}.issubset(df.columns):
        # Label: fragile = 1 if a flip is detected within budget (r <= BUDGET);
        # missing flips (NaN) are treated as 0 (non-fragile).
        ...
        if np.isfinite(scores).any() and y_true.sum() > 0:  # at least one positive present
            rec, prec, auprc_c4, df_curve = precision_recall_from_scores(y_true, scores)

            # Compute F1 along the curve (skip the prepended (1,0) point).
            ...
            # Legacy top-K point: choose K = #positives, then precision==recall at that prefix.
            K = int(y_true.sum())
            if K > 0:
                tp_at_k = int(df_curve.loc[min(K, len(df_curve)) - 1, "tp"])
                pr_at_topk = tp_at_k / K
                re_at_topk = tp_at_k / K  # at top-K: precision == recall
    ...
    def _auprc_for_score(name, scores, y_true, out_prefix):
        """
        Helper (scoped to `main`): compute AUPRC for a given score vector,
        persist the PR table and figure, and return AP.

        Parameters
        ----------
        name : str
            Label used in the plot legend/title.
        scores : array-like
            Score values; higher = more likely positive.
        y_true : np.ndarray
            Binary ground truth vector used earlier.
        out_prefix : str
            Filename stem for CSV/PNG.

        Returns
        -------
        float
            Average precision for the given score, or NaN if undefined.
        """
    ...
    results_pr = []
    # Include already-computed |c4| into the comparison table if available
    if np.isfinite(auprc_c4):
        results_pr.append(("|c4|", float(auprc_c4)))

    EPS = 1e-9

    # 1/r scores derived from radii parsed from TXT
    for col, label, pref in [
        ...
        if col in df.columns:
            ...
            score = 1.0 / np.where(np.isfinite(r), r + EPS, np.inf)  # NaN -> 0 via inf denominator
            ap = _auprc_for_score(label, score, y_true, pref)
            results_pr.append((label, ap))

    # grad_norm extracted from Saliency logs (if available)
    ...
    # Save the summary comparison of AUPRC across scores
    ...
    # 7) Figures
    # 7a) r_dom vs r_flip with a simple regression line (and y=x reference)
    plt.figure(figsize=(6, 5))
    if len(sub) > 0:
        plt.scatter(sub["r_dom"], sub["r_flip_obs"], alpha=0.85)
        # Not enforcing origin; use standard least squares with small ridge.
        try:
